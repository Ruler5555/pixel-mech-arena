...
(function () {
  ...
  const CARD_META = {
    ...
    potion: { emoji: '🧪', name: '恢复药水', eff: '立即恢复20%血量', left: '每场2次·血量<60%出现' },
  ...
  const CHANGELOG = [
    ...
    ['v196', [
      '等效强制中继:只给TURN无STUN,走北京节点稳定40-50ms',
      '根治P2P抖动:PeerJS relay策略bug已绕开(无头浏览器实测)'
    ]],
    ['v195', [
      'AI观战端接入缓冲插值:特效不再抖动卡顿',
      '回退15Hz档恢复30Hz(观战体验回升)'
    ]],
    ['v194', [
    ...
    ['v192', [
      '回退v189-v191强制中继(P2P优先+自建TURN兜底)',
      '强制中继浏览器侧通道建立问题待后续深挖'
    ]],
    ['v188', [
    ...
    ['v187', [
      '自建TURN优先(中继走北京节点20-40ms)',
      '修复:之前动态拉取Metered时自建没被用上'
    ]],
    ['v186', [
      '自建国内TURN中继上线(北京节点UDP 3478实测通)',
      '跨网中继延迟从1000ms+砸到10-30ms,替换jp兜底'
    ]],
    ['v185', [
      '中继降频10Hz降低重传压力(链路1000ms+场景堆积更少)',
      'TURN兜底UDP前置(低延迟优先, TCP/TLS保留兜底); 节点实测jp仍最优不换'
    ]],
    ['v184', [
    ...
    ['v179', [
      '中继检测后自动重试直连(等待期重建连接重新协商)',
      '对称NAT映射动态变化, 重抽签有机会拿到可穿透映射直连'
    ]],
    ['v178', [
      '穿透优化: P2P重试前2轮8s后20s长轮(对齐v109长宽限, 慢速协商不再被打断)',
      '对称NAT+TURN慢速场景协商8-20s, 8s重开会打断 → 20s窗口提升穿透成功率'
    ]],
    ['v177', [
      '根治中继仍1000ms: 限速改RTT驱动(bufferedAmount/getStats移动端失效)',
      '高RTT自动降15Hz自我修正: 队列清→延迟回落, 直连低延迟仍30Hz'
    ]],
    ['v176', [
      '移除v175接入的死TURN(43.138.235.180实测无响应), 保留日本节点',
      '修正v175漏改的12处?v=(仅versionTag升, 脚本缓存未失效)'
    ]],
    ['v175', [
      '接入国内免费TURN(腾讯云北京, ping~43ms): 优先于日本节点',
      'TURN兜底顺序: 国内免费服 → 日本 → 新加坡 → global'
    ]],
    ['v174', [
